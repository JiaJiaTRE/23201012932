<template>
    <div class="box">
        ...
        <div class="box-body">
            ...
            <div class="food">
                <div>
                    <img class="box-img" :src="getImg(data.foods.pic)">
                    <div class="foods-title">
                        <div>
                            <span>{{ data.foods.foodname }}</span>
                            <span>￥{{ data.foods.price }}</span>
                        </div>
                        <div>
                            <div @click="minus">-</div>
                            <span class="foods-num">{{ data.num }}</span>
                            <div @click="add">+</div>
                        </div>
                    </div>
                </div>
...
import { getFoodByIdApi, insertOrderApi } from "../api/index.js"
...
const sum = computed(() => {
    return data.num * data.foods.price + packing.value - certificate.value
})
...
const getFoodById = async (e) => {
    const res = await getFoodByIdApi(e);
    data.foods = res;
}

const insertOrder = async (e) => {
    const res = await insertOrderApi(e);
    if (res.success === '0') {
        showFailToast('购买失败！')
    }
    if (res.success === '1') {
...
onMounted(() => {
    getFoodById(route.query.foodId);
})
